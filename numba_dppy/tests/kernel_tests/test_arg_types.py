...
def test_kernel_arg_types(filter_str, input_arrays):
    kernel = numba_dppy.kernel(mul_kernel)
...
def test_bool_type(filter_str):
    kernel = numba_dppy.kernel(check_bool_kernel)
    a = np.array([2], np.int64)

    device = dpctl.SyclDevice(filter_str)
    with dpctl.device_context(device):
        kernel[a.size, numba_dppy.DEFAULT_LOCAL_SIZE](a, True)
        assert a[0] == 111
        kernel[a.size, numba_dppy.DEFAULT_LOCAL_SIZE](a, False)
        assert a[0] == 222
