...
def kernel_result_pair(request):
    op = getattr(numba_dppy.atomic, request.param[0])

    def f(a):
        op(a, 0, 1)

    return numba_dppy.kernel(f), request.param[1]


skip_no_atomic_support = pytest.mark.skipif(
    not numba_dppy.ocl.atomic_support_present(),
...
def test_kernel_atomic_simple(filter_str, input_arrays, kernel_result_pair):
    ...
    with dpctl.device_context(device):
        kernel[global_size, numba_dppy.DEFAULT_LOCAL_SIZE](a)
    assert a[0] == expected
...
def get_func_global(op_type, dtype):
    ...
    """
    op = getattr(numba_dppy.atomic, op_type)
...
def get_func_local(op_type, dtype):
    ...
    def f(a):
        lm = numba_dppy.local.array(1, dtype)
        lm[0] = a[0]
        numba_dppy.barrier(numba_dppy.CLK_GLOBAL_MEM_FENCE)
        op(lm, 0, 1)
        numba_dppy.barrier(numba_dppy.CLK_GLOBAL_MEM_FENCE)
        a[0] = lm[0]
...
def test_atomic_fp_native(
    ...
    kernel = numba_dppy.kernel(function)
    ...
    with override_config("NATIVE_FP_ATOMICS", NATIVE_FP_ATOMICS):
        ...
        with dpctl.device_context(filter_str) as sycl_queue:
            ...
            specialized_kernel = kernel[
                global_size, numba_dppy.DEFAULT_LOCAL_SIZE
