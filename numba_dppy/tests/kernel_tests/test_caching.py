...
def test_caching_kernel_using_same_queue(filter_str):
    ...
    with dpctl.device_context(filter_str) as gpu_queue:
        func = numba_dppy.kernel(data_parallel_sum)
        cached_kernel = func[global_size, numba_dppy.DEFAULT_LOCAL_SIZE].specialize(
            func._get_argtypes(a, b, c), gpu_queue
        )

        for i in range(10):
            _kernel = func[global_size, numba_dppy.DEFAULT_LOCAL_SIZE].specialize(
                func._get_argtypes(a, b, c), gpu_queue
            )
